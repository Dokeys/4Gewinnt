...
enum field_placement_t {
	EMPTY, CHIP_PLAYER_1, CHIP_PLAYER_2
};
typedef enum field_placement_t field_placement_t;
...
static bool set_chip(field_placement_t (*board) [BOARD_SIZE_Y], field_placement_t current_player, uint8_t selection_position); 
...
/* public functions */
void start_game() {
	/* In this variable the chip positions is saved. Zero on the Y field is the lowes and zero on the 
	X field is the left one.*/
		...
		for(uint8_t y=0; y<=BOARD_SIZE_Y-CHIP_NUMBER_TO_WIN; y++) {
			if(*(*(board+x)+y) != EMPTY) {
				...
				for(uint8_t i=0; i<CHIP_NUMBER_TO_WIN; i++) { /* up and right */
					if((x+i+1<=BOARD_SIZE_X) /* check if placement check is still in the field */ && (*(*(board+x+i+1)+y+i+1)==chip_in_current_field) ) {
						chips_in_a_row++;
					} else {
